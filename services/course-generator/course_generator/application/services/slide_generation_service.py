...
from course_generator.domain.entities.course_content import Slide, SlideType, DifficultyLevel
from course_generator.domain.interfaces.content_generation_service import ISlideGenerationService
from data_access.course_generator_dao import CourseGeneratorDAO
from course_generator.domain.interfaces.ai_service import IAIService, ContentGenerationType, IPromptTemplateService
...
class SlideGenerationService(ISlideGenerationService):
    ...
    async def generate_slides(self, course_id: str, title: str, description: str, 
        ...
        saved_slides = await self._course_generator_dao.bulk_create(slides)
        return saved_slides
    ...
    async def generate_single_slide(self, course_id: str, topic: str, 
        ...
        existing_slides = await self._course_generator_dao.get_by_course_id(course_id)
        slide.order = len(existing_slides) + 1
        
        # Save slide
        return await self._course_generator_dao.create(slide)
    ...
    async def regenerate_slide(self, slide_id: str, new_focus: str = None, 
        ...
        # Get existing slide
        existing_slide = await self._course_generator_dao.get_by_id(slide_id)
        ...
        # Save updated slide
        return await self._course_generator_dao.update(existing_slide)
