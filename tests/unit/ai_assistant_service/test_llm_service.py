...
class TestLLMService:
    ...
    def openai_service(self):
        ...
        """
        with patch('ai_assistant_service.application.services.llm_service.openai'):
    ...
    def claude_service(self):
        ...
        """
        with patch('ai_assistant_service.application.services.llm_service.anthropic'):
    ...
    async def test_generate_completion_success(self, openai_service, sample_messages):
        ...
        # Arrange
        mock_response = Mock()
        mock_response.choices = [Mock(message=Mock(content="Python 101 is required."))]
        mock_response.usage = Mock(total_tokens=50)

        with patch.object(openai_service, '_call_openai_api', return_value=mock_response):
            # Act
    ...
    async def test_generate_completion_with_functions(self, openai_service, sample_messages):
        ...
        mock_response.choices = [Mock(
            message=Mock(
                content=None,
                function_call=Mock(name="get_course_info", arguments='{"course_id": "123"}')
            )
        )]
    ...
    async def test_generate_completion_api_error(self, openai_service, sample_messages):
        ...
        """
        # Arrange
        with patch.object(openai_service, '_call_openai_api', side_effect=Exception("API Error")):
            # Act & Assert
    ...
    async def test_generate_streaming_completion(self, openai_service, sample_messages):
        ...
        # Arrange
        mock_chunks = [
            Mock(choices=[Mock(delta=Mock(content="Python "))]),
            Mock(choices=[Mock(delta=Mock(content="101 "))]),
            Mock(choices=[Mock(delta=Mock(content="required."))])
        ]
        ...
        with patch.object(openai_service, '_call_openai_streaming_api', return_value=mock_stream()):
            # Act
            chunks = []
    ...
    async def test_rate_limit_handling(self, openai_service, sample_messages):
        ...
        with patch.object(openai_service, '_call_openai_api', side_effect=side_effect):
            # Act
    ...
    async def test_timeout_handling(self, openai_service, sample_messages):
        ...
        """
        # Arrange
        with patch.object(openai_service, '_call_openai_api', side_effect=TimeoutError()):
            # Act & Assert
...
class TestLLMServiceIntegrationScenarios:
    ...
    async def test_multi_turn_conversation(self):
        ...
        mock_response = Mock()
        mock_response.choices = [Mock(message=Mock(content="Start with Python 101."))]
        mock_response.usage = Mock(total_tokens=100)

        with patch.object(service, '_call_openai_api', return_value=mock_response):
            # Act
    ...
    async def test_function_calling_workflow(self):
        ...
        mock_response_1.choices = [Mock(
            message=Mock(
                content=None,
                function_call=Mock(name="get_course", arguments='{"course_id": "123"}')
            )
        )]

        # Second call returns final answer
        mock_response_2 = Mock()
        mock_response_2.choices = [Mock(message=Mock(content="Course 123 is Python 101."))]
        mock_response_2.usage = Mock(total_tokens=50)

        with patch.object(service, '_call_openai_api', side_effect=[mock_response_1, mock_response_2]):
            # Act
