...
from domain.entities.password_policy import PasswordPolicy, PasswordStrength
...
class TestUser:
    ...
    def test_user_has_role_returns_correct_boolean(self):
        ...
        # Act & Assert
        assert student_user.has_role(Role.STUDENT)
        assert not student_user.has_role(Role.INSTRUCTOR)
        assert not student_user.has_role(Role.ADMIN)
        
        assert instructor_user.has_role(Role.INSTRUCTOR)
        assert not instructor_user.has_role(Role.STUDENT)
        assert not instructor_user.has_role(Role.ADMIN)
