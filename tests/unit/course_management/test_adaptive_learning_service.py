...
def mock_dao():
    """
    WHAT: Creates a mocked LearningPathDAO
    WHERE: Used by all service tests
    WHY: Isolates service logic from database operations
    """
    dao = MagicMock(spec=LearningPathDAO)
    # Make all methods async
    dao.create_learning_path = AsyncMock()
    dao.get_learning_path_by_id = AsyncMock()
    dao.get_learning_paths_by_student = AsyncMock()
    dao.update_learning_path = AsyncMock()
    dao.delete_learning_path = AsyncMock()
    dao.create_node = AsyncMock()
    dao.update_node = AsyncMock()
    dao.get_nodes_by_path = AsyncMock()
    dao.create_prerequisite_rule = AsyncMock()
    dao.get_prerequisites_for_content = AsyncMock()
    dao.delete_prerequisite_rule = AsyncMock()
    dao.create_recommendation = AsyncMock()
    dao.get_pending_recommendations = AsyncMock()
    dao.update_recommendation = AsyncMock()
    dao.upsert_mastery_level = AsyncMock()
    dao.get_mastery_levels_by_student = AsyncMock()
    dao.get_skills_needing_review = AsyncMock()

    # Mock pool for direct queries in update_node_progress
    mock_pool = MagicMock()
    mock_conn = AsyncMock()
...
def service(mock_dao):
    """
    WHAT: Creates AdaptiveLearningService with mocked DAO
    WHERE: Used by all service tests
    WHY: Provides service instance for testing
    """
    return AdaptiveLearningService(
        learning_path_dao=mock_dao,
        course_dao=None,
